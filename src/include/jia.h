 ...
 *
 * @note jia_barrier() cannot be called inside a critical section enclosed by
 * jia_lock() and jia_unlock()
 */
void jia_barrier();
...
/**
 * @brief jia_error -- print out the err string and shut down all processes
 * started by jia_init()
 *
 * @param ...
 */
...
/**
 * @brief jia_recv -- an MPI-similar call, receive len bytes from host frompid
 * to buf
 *
 * @return int
 */
...
/**
 * @brief jia_reduce -- an MPI-like call, reduce cnt numbers from all hosts to
 * host root with operation op
 *
 */
void jia_reduce(char *snd, char *rcv, int cnt, int op, int root);

/**
 * @brief jia_bcast -- an MPI-similar call, send len bytes from buf of host root
 * to buf of all hosts
 *
 */
 ...
 *
 * @note turn on/off optimization methods, such as home migration, write vector,
 * adaptive write detection
 */
