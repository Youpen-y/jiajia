...
typedef struct wtnttype {
        ...
        int             from[Maxwtnts];         // modify addr's frompid
        int             scope[Maxwtnts];        // addr's scope(record version when is lock)
        int             mig[Maxwtnts];          // addr's mig(record whether cache is valid when is barrier)
