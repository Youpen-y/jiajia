...
void msgrecvserver(jia_msg_t *req)
{
  ...
  i=0;
  while ((msgbuf[i].op!=ERRMSG)&&(i<Maxmsgbufs)) i++;   // find the first msg whose op != ERRMSG
...
/**
 * @brief nextpacket -- find next packet in the msgbuf that meet fromproc and tag condition
 * 
 * @param fromproc 
 * @param tag 
 * @return int 
