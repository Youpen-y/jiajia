...
/*
 */
#define jia_mbarrier jia_barrier
...
struct GlobalMemory {
    ...
    unsigned int done;
    int id;
    int start;  /*BARDEC(start)*/
    int idlock; /*LOCKDEC(idlock)*/
} * Global;
...
void daxpy1(double *, double *, int, double);
...
int main(int argc, char **argv) {
    ...
    */
    ...
    while ((ch = getopt(argc, argv, "n:p:b:cstoh")) != -1) {
        ...
        case 'h':
            ...
            printf("  -bB : Use a block size of B. BxB elements should fit in "
                   "cache for \n");
            printf("        good performance. Small block sizes (B=8, B=16) "
                   "work well.\n");
            printf("  -c  : Copy non-locally allocated blocks to local memory "
                   "before use.\n");
    ...
    printf(" out barrier!\n");
    for (i = 0; i < P; i++) {
        last_malloc[i] = (double *)jia_alloc2p(proc_bytes[i] + PAGE_SIZE, i);
    }
    printf(" enter barrier!\n");
    jia_barrier();
    printf(" out barrier!\n");
    printf("***************************\n");
    local_malloc = (unsigned long *)malloc(P * sizeof(long));
    for (i = 0; i < P; i++) {
        local_malloc[i] = 0;
    }
    for (i = 0; i < P; i++) {
        local_malloc[i] = (unsigned long)last_malloc[i];
    }
    ...
    for (i = 0; i < nblocks; i++) {
        for (j = 0; j < nblocks; j++) {
            proc_num = BlockOwner(i, j);
            a[i + j * nblocks] = (double *)local_malloc[proc_num];
            if ((i == nblocks - 1) && (j == nblocks - 1)) {
                size = edge * edge;
            } else if ((i == nblocks - 1) || (j == nblocks - 1)) {
                size = edge * block_size;
    ...
    */
    ...
    fprintf(stdout, "Elapsed time: %.2f seconds\n",
            (((finisht.tv_sec * 1000000.0) + finisht.tv_usec) -
             ((startt.tv_sec * 1000000.0) + startt.tv_usec)) /
                1000000.0);
    ...
    if (jiapid == 0) {
        ...
        printf("              Total      Diagonal     Perimeter      Interior  "
               "     Barrier\n");
        printf(" Proc         Time         Time         Time           Time    "
               "      Time\n");
        printf("    0    %10.0f    %10.0f    %10.0f    %10.0f    %10.0f\n",
               Global->completion[0], Global->t_in_fac[0],
               Global->t_in_solve[0], Global->t_in_mod[0], Global->t_in_bar[0]);
        if (dostats) {
            for (i = 1; i < P; i++) {
                printf(
                    "  %3d    %10.0f    %10.0f    %10.0f    %10.0f    %10.0f\n",
...

{
    ...
    printf(" Enter Slavestart!\n");
    OneSolve(n, block_size, a, MyNum, dostats);
    printf(" Out Slavestart!\n");
}
...
void lu0(double *a, int n, int stride) {
    ...
    for (k = 0; k < n; k++) {
        ...
        for (j = k + 1; j < n; j++) {
            ...
            alpha = -a[k + j * stride];
            length = n - k - 1;
            daxpy(&a[k + 1 + j * stride], &a[k + 1 + k * stride], n - k - 1,
                  alpha);
        }
    }
}
